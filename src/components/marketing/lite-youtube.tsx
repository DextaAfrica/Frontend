 ...
 *
 * Framed to match the site's other rounded-panel media (`MediaFrame`,
 * `project-card`): a hairline border, clipped corners, a slow poster zoom on
 * hover while it's still a facade.
 */
export function LiteYouTube({
  ...
  return (
    <Reveal
      as="figure"
      className={cn(
        "group/video relative aspect-video overflow-hidden rounded-panel border border-border bg-muted",
        className,
      )}
    >
      {playing ? (
        ...
        >
          ...
          <span
            aria-hidden
            className="lite-youtube__play absolute top-1/2 left-1/2 grid size-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-primary text-primary-foreground shadow-[var(--control-shadow-glow)] transition-transform duration-[240ms] ease-premium group-hover/video:scale-110"
          >
